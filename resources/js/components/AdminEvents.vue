  ...
  <div class="container mx-auto">
            ...
            <tbody class="bg-white">
              <tr class="event-row" v-for="event in events" :key="event.id" @click="location(`/admin/events/${event.id}`)" style="cursor: pointer;">
                ...
                <td class="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                  <div class="text-sm leading-5 text-gray-500 text-center">{{ (event.last_registration_at || '').slice(0, 10) }}</div>
                </td>
    ...
    <form class="mt-4 mb-6 max-w-xl mx-auto" v-show="showNewEvent">
      <div>
        ...
        <input v-model="event.name" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" name="name">
      </div>

      <div>
        <div class="text-lg font-thin mt-2">Datum</div>
        <input v-model="event.date" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" type="date" name="date">
      </div>

      <div>
        <div class="text-lg font-thin mt-2">Ev. sista anmälningsdag</div>
        <input v-model="event.last_registration_at" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" type="date">
      </div>

      <div class="mt-2">
        <input v-model="event.time" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" name="time" placeholder="Ungefärlig tid, ex. 8 – 15">
      </div>
      <div class="mt-2">
      ...
      <div class="mt-2">
        <textarea v-model="event.description" rows="5" placeholder="Ev. ytterligare info" class="form-textarea block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5"></textarea>
      </div>
      ...
      <div class="flex mb-2 items-center">
        ...
        <select v-model="event.show_status" class="mt-1 block form-select w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5">
