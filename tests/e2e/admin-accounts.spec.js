...
test('The admin can inactivate a member without survey email', async ({ page }) => {
  ...
  const requestPromise = page.waitForRequest(
    (request) => request.url().includes(`/admin/accounts/inactivate/${user.id}`) && request.method() === 'POST',
  )
...
test('The admin can set and clear background check date', async ({ page }) => {
  ...
  await page.getByTestId(`settings-${user.id}`).click()
  await page
    .locator('[data-testid="background-check-date"]')
    .evaluate((el) => el.closest('.flex').querySelector('button').click())
