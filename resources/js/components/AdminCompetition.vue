        ...
        <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
          <table data-testid="competition-table" class="min-w-full">
            ...
            <tbody class="bg-white">
              <tr v-for="registration in sortedRegistrations" :key="registration.id" :data-testid="`registration-${registration.id}`">
...
export default {
  ...
  computed: {
    ...
    filteredRegistrations() {
      ...
      return this.competition.registrations.filter((registration) => registration.status == this.showFilter)
    },
    countYes() {
      return this.competition.registrations.filter((registration) => registration.status == 1).length
    },
    countNo() {
